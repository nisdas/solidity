...
public:
	...
	virtual void visit(Statement& _statement) override;
...
private:
	...
	/// Visits a statement, but saves and restores the current statements to prefix and returns
	/// the statements that should be prefixed for @a _statement.
	std::vector<Statement> visitRecursively(Statement& _statement);
